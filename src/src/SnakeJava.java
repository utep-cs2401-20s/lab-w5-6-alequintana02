...
/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
    ...
    public int neighbor(int r, int c) {//counts LIVE neighbors//make sure to not go out of bounds
        int neighbor = 0;
        if (game[r][c - 1]) {//left
            neighbor++;
...
// position of the tail in the grid, and the length of the snake on the board. Increments exhaustiveChecks counter with each (x',y') cell that is examined.
            ...
            for (int c = 0; c < game[r].length; c++) {//col traversal
                ...
                if (game[r][c]) {//we want to make sure the cell is true
                    ...
                    if(neighbors == 1) {//could be head or could be tail
                        if (!game[headPosition[0]][headPosition[1]]) {//if it is not at HP then it is part of the tail
                            tailX = r;
                            tailY = c;
                            result[0] = tailX;
                            result[1] = tailY;
    ...
    private int[] findTailRecursive (int[] currentPosition, int[] previousPosition) {
        ...
        int length = 1;// includes head count
        int tailX = 0;
        int tailY = 0;
        int temp[];
        ...
        if (neighborsCurrent == 1 && currentPosition == previousPosition) {
            ...
            //all of this is to update current and previous positions
